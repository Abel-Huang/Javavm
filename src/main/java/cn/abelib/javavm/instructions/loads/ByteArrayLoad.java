 ...
 * @date 2023/7/4 22:57
 */
public class ByteArrayLoad extends NoOperandsInstruction implements ArrayInstruction {
    @Override
    public void execute(Frame frame) {
        ...
        checkNotNil(arrRef);
        List<Integer> bytes = arrRef.getBytes();
        checkIndex(bytes.size(), index);
        stack.pushInt(bytes.get(index));
    }
}
